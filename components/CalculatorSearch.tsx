...
export function CalculatorSearch({ calculators, placeholder = "Search calculators..." }: Props) {
  ...
  return (
    <div className="relative w-full max-w-md" role="combobox" aria-expanded={filtered.length > 0} aria-haspopup="listbox">
      <input
        type="text"
        value={query}
      ...
      {filtered.length > 0 && (
        <div
          id="search-results"
          role="listbox"
          className="absolute top-full mt-2 w-full rounded-xl border border-gray-100 dark:border-gray-700 bg-white dark:bg-gray-800 p-2 shadow-lg z-50"
        >
          {filtered.map((c) => (
            <Link
              key={c.slug}
