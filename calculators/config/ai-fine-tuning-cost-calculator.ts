...
const config = {
  ...
  content: {
    intro: "Fine-tuning an AI model tailors a pre-trained foundation model to your specific domain, use case, or brand voice. While the base models are capable, fine-tuning can dramatically improve performance on specialized tasks. However, the costs add up  -  you pay for training compute, then ongoing inference at fine-tuned model rates. This calculator helps you budget for the total cost of fine-tuning including both the one-time training and ongoing inference.",
